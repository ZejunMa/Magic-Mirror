...
def process(pose_est_results, pred_3d_instances):
    coordinates2d = (pose_est_results[0].__dict__)['_pred_instances']['keypoints'][0]
    keypoint_positions = (pred_3d_instances.__dict__)['keypoints'][0]

    projected2d = coordinates2d
    projected3d = keypoint_positions[:, :2]
    # projected3d = projected3d.dot(np.array([[-1, 0],[0, 1]]))
    y = projected2d[[6, 8, 10, 5, 7, 9, 12, 14, 16, 11, 13, 15], :]
    x = projected3d[[11, 12, 13, 14, 15, 16, 4, 5, 6, 1, 2, 3], :]
    x = np.insert(x, 2, np.ones(12), axis=1)
    c = (np.linalg.inv((x.T.dot(x))).dot(x.T)).dot(y)

    projected3d = np.insert(projected3d, 2, np.ones(17), axis=1)
    keypoint_positions = projected3d.dot(c)
...
def process_one_image(args, detector, frame, frame_idx, pose_estimator,
    ...
    # # 提取 x 和 y 坐标
    # xy = pose_est_results[0].__dict__['_pred_instances']['keypoints'][0]
    # x = xy[:,0]
    # y = xy[:,1]
    # # 创建一个散点图
    # plt.scatter(x, y)
...
def main():
    ...
    if input_type == 'image':
        ...
        # keypoints = process(pose_est_results, pred_3d_instances)
        # import matplotlib.pyplot as plt
        # # 提取 x 和 y 坐标
        # xy = pose_est_results[0].__dict__['_pred_instances']['keypoints'][0]
        # # xy = keypoints
        ...
        while video.isOpened():
            ...
            try:
                ...
                height_2d = np.max(coordinates2d[:, 1]) - np.min(coordinates2d[:, 1])
                height_3d = np.max(keypoint_positions[:, 2]) - np.min(keypoint_positions[:, 2])
                kp_scale = height_2d / height_3d
                keypoint_positions *= kp_scale

                vector = keypoint_positions[1]
                keypoint_positions = keypoint_positions - vector

                640 * 425
                coordinates2d[:, 1] = 425 - coordinates2d[:, 1]
                # coordinates2d[:, 1] = coordinates2d[:, 1] - 425 / 2
                # coordinates2d[:, 0] = coordinates2d[:, 2] - 320
                vector2d = coordinates2d[11]
                vector2d[0] -= 320
                vector2d[0] *= -1
                vector2d[1] -= 425 / 2
                keypoint_positions += np.insert(vector2d, 1, 1)
                # keypoint_positions *= [-1, -1, 1]
                ...
                # keypoint_positions = projected3d.dot(c)
                #
                # np.array(keypoint_positions)

                keypoint_positions = keypoint_positions.tolist()
                message = json.dumps(keypoint_positions)
                # time.sleep(0.05)

                socket.send_string(message)
                print("Sent data to Unity.")
            finally:
                socket.close()
                context.term()
